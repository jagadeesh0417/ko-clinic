      ...
      <div className="container-custom">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-16">
          {locations.map((region, regionIndex) => (
            ...
            >
              <h3 className="font-heading text-2xl text-[#C9A86A] mb-6">
                {region.country}
              </h3>
                ...
                {region.cities.map((city) => (
                  <GlassCard key={city.name} hover={false}>
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <h4 className="font-heading text-lg text-[#FAFAFA]">{city.name}</h4>
                        <p className="text-[#A1A1AA] text-sm font-body mt-1">{city.address}</p>
                      </div>
                    </div>
                  </GlassCard>
                ))}
